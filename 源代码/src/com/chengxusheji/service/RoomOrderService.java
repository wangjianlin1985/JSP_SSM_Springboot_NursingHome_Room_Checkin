...
public class RoomOrderService {
    ...
    /*按照查询条件分页查询订单记录*/
    public ArrayList<RoomOrder> userQueryRoomOrder(String user_name,Room roomObj,OldMan oldManObj,String liveDate,String orderState,String orderTime,int currentPage) throws Exception { 
     	String where = "where t_userInfo.user_name='" + user_name + "' ";
    	if(null != roomObj &&  roomObj.getRoomNo() != null  && !roomObj.getRoomNo().equals(""))  where += " and t_roomOrder.roomObj='" + roomObj.getRoomNo() + "'";
    	if(null != oldManObj && oldManObj.getOldManId()!= null && oldManObj.getOldManId()!= 0)  where += " and t_roomOrder.oldManObj=" + oldManObj.getOldManId();
    	if(!liveDate.equals("")) where = where + " and t_roomOrder.liveDate like '%" + liveDate + "%'";
    	if(!orderState.equals("")) where = where + " and t_roomOrder.orderState like '%" + orderState + "%'";
    	if(!orderTime.equals("")) where = where + " and t_roomOrder.orderTime like '%" + orderTime + "%'";
    	int startIndex = (currentPage-1) * this.rows;
    	return roomOrderMapper.userQueryRoomOrder(where, startIndex, this.rows);
    }
    ...
    /*当前查询条件下计算总的页数和记录数*/
    public void userQueryTotalPageAndRecordNumber(String user_name,Room roomObj,OldMan oldManObj,String liveDate,String orderState,String orderTime) throws Exception {
     	String where = "where t_userInfo.user_name='" + user_name + "'";
    	if(null != roomObj &&  roomObj.getRoomNo() != null && !roomObj.getRoomNo().equals(""))  where += " and t_roomOrder.roomObj='" + roomObj.getRoomNo() + "'";
    	if(null != oldManObj && oldManObj.getOldManId()!= null && oldManObj.getOldManId()!= 0)  where += " and t_roomOrder.oldManObj=" + oldManObj.getOldManId();
    	if(!liveDate.equals("")) where = where + " and t_roomOrder.liveDate like '%" + liveDate + "%'";
    	if(!orderState.equals("")) where = where + " and t_roomOrder.orderState like '%" + orderState + "%'";
    	if(!orderTime.equals("")) where = where + " and t_roomOrder.orderTime like '%" + orderTime + "%'";
        recordNumber = roomOrderMapper.userQueryRoomOrderCount(where);
        int mod = recordNumber % this.rows;
        totalPage = recordNumber / this.rows;
        if(mod != 0) totalPage++;
    }
