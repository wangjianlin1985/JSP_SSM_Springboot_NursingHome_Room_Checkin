...
public class OldManController extends BaseController {
	...
	/*用户通过客户端ajax方式提交添加老人信息*/
	@RequestMapping(value = "/userAdd", method = RequestMethod.POST)
	public void userAdd(OldMan oldMan, BindingResult br,
		...
		oldMan.setUserObj(userObj);
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		oldMan.setAddTime(sdf.format(new java.util.Date()));
	...
	public void userListAll(HttpServletResponse response,HttpSession session) throws Exception {
		...
		List<OldMan> oldManList = oldManService.userQueryAllOldMan(user_name);
        response.setContentType("text/json;charset=UTF-8"); 
		PrintWriter out = response.getWriter();
		JSONArray jsonArray = new JSONArray();
		for(OldMan oldMan:oldManList) {
	...
	public String frontUserlist(String name,@ModelAttribute("userObj") UserInfo userObj,String addTime,Integer currentPage, Model model, HttpServletRequest request,HttpSession session) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (name == null) name = "";
		if (addTime == null) addTime = "";
	    ...
	    List<UserInfo> userInfoList = userInfoService.queryAllUserInfo();
	    request.setAttribute("userInfoList", userInfoList);
		return "OldMan/oldMan_frontUserquery_result"; 
	}
