...
public class RoomOrderController extends BaseController {
	...
	public void statistic(HttpServletRequest request,HttpServletResponse response) throws Exception {
    	...
    	for(int i=1;i<=12;i++) {
    		...
    		ArrayList<RoomOrder> roomOrderList = roomOrderService.queryRoomOrder(null, null, monthString, "", "");
    		for(RoomOrder roomOrder: roomOrderList) {
    			monthMoney += roomOrder.getOrderMoney();
    		}
    		xData.put(i+"月");
	...
	public String frontUserlist(@ModelAttribute("roomObj") Room roomObj,@ModelAttribute("oldManObj") OldMan oldManObj,String liveDate,String orderState,String orderTime,Integer currentPage, Model model, HttpServletRequest request,HttpSession session) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (liveDate == null) liveDate = "";
		if (orderState == null) orderState = "";
		if (orderTime == null) orderTime = "";
		String user_name = session.getAttribute("user_name").toString();
		List<RoomOrder> roomOrderList = roomOrderService.userQueryRoomOrder(user_name,roomObj, oldManObj, liveDate, orderState, orderTime, currentPage);
	    /*计算总的页数和总的记录数*/
	    roomOrderService.userQueryTotalPageAndRecordNumber(user_name,roomObj, oldManObj, liveDate, orderState, orderTime);
	    /*获取到总的页码数目*/
	    int totalPage = roomOrderService.getTotalPage();
	    /*当前查询条件下总记录数*/
	    int recordNumber = roomOrderService.getRecordNumber();
	    ...
	    List<Room> roomList = roomService.queryAllRoom();
	    request.setAttribute("roomList", roomList);
		return "RoomOrder/roomOrder_frontUserquery_result"; 
	}
